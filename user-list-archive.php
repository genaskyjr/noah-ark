...
if(isset($_GET['search'])){
    $keyword = '%' . $_GET['search'] . '%';
    $stmt = $conn->prepare("SELECT `id`, `is_role`, `reg_date`, `fullname`, `email`, `address`, `phone_number`, is_archive
    FROM users
    WHERE (fullname LIKE :keyword OR email LIKE :keyword OR address LIKE :keyword OR phone_number LIKE :keyword) 
    AND is_archive = 1");
  ...
  //to not show admin
  $stmt = $conn->prepare("SELECT * FROM `users` WHERE is_role != 'admin' AND is_archive = '1'");
...
<td><?php echo $row['fullname'];  ?></td>
      ...
      <td>
        <a  href="profile.php?fullname=<?php echo $row['fullname'] . '&address=' . $row['address'] . '&email=' . $row['email'] . '&phone_number=' . $row['phone_number'] . '&pet_count=' . $count ?>" class="btn btn-primary btn-md"><i class="fa-solid fa-eye"></i>View</a>
        <a  href="account.php?id=<?php echo $row['id']?>" class="btn btn-success btn-md"><i class="fa-solid fa-pen-to-square"></i>Edit </a>        
        <button type="button" onclick="deleteUser(<?php echo $row['id'] ?>)" class="btn btn-secondary btn-md"><i class="fa-solid fa-rotate-right"></i> Archive</button>
        </td>
