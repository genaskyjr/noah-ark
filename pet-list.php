...
<td><img  src="<?php echo $row['pet_img'];  ?>" alt="Logo" width="70" height="70" class="d-inline-block align-text-top"></td>
        ...
        <td>
        <a href="view-pet.php?pet_name=<?php echo $row['pet_name'] . '&pet_birthday=' . $row['pet_birthday'] . '&pet_age=' . $age . '&pet_last_vaccine=' . $row['pet_last_vaccine'] . '&pet_next_vaccine=' . $row['pet_next_vaccine'] . '&pet_img=' . $row['pet_img'] . '&pet_gender=' . $row['pet_gender'] . '&pet_type=' . $row['pet_type']; ?>" class="btn btn-primary btn-md"><i class="fa-solid fa-eye"></i>View </a> 
        <a href="update-pet.php?pet_id=<?php echo $row['pet_id'] ?>" class="btn btn-success btn-md"><i class="fa-solid fa-pen-to-square"></i> Edit </a>
        <button class="btn btn-danger btn-md" onClick="delete_pet(<?php echo $row['pet_id']; ?>, '<?php echo $row['pet_img']; ?>')"><i class="fa-solid fa-box-archive"></i>Archive  </button>
...
function delete_pet(petId, pet_img) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            xhr.send("pet_id=" + petId + "&pet_img=" + pet_img);
        }
    })
}
</script>
