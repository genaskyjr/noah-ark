  ...
  <thead>
    <tr>
     <!-- <th scope="col">Action</th>
      ...
      <th scope="col">Date of Rescued</th>
...
if(isset($_GET['search'])){
  $keyword = '%' . $_GET['search'] . '%';
  $stmt = $conn->prepare("SELECT `adoption_number`, `adoption_gender`, `adoption_recued`, `adoption_type`, `adoption_image`, `adoption_nickname`
    FROM adoption_pets
    WHERE (adoption_number LIKE '$keyword'
        OR adoption_gender LIKE '$keyword'
        OR adoption_nickname LIKE '$keyword'
        OR adoption_recued LIKE '$keyword'
        OR adoption_type LIKE '$keyword')
    AND is_archive = 1");
  ...
  $stmt = $conn->prepare("SELECT * FROM `adoption_pets` WHERE is_archive = 1");
  $stmt->execute(); // Execute the statement first.
  $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
<td><img  src="<?php echo $row['adoption_image'];  ?>" alt="Logo" width="50" height="50" class="d-inline-block align-text-top"></td>
      

        <td>
        <a href="view-adoption-pet.php?adoption_number=<?php echo $row['adoption_number'] . '&adoption_gender=' . $row['adoption_gender'] . '&adoption_recued=' . $row['adoption_recued'] . '&adoption_type=' . $row['adoption_type'] . '&adoption_image=' . $row['adoption_image']; ?>" class="btn btn-primary btn-md"><i class="fa-solid fa-eye"></i>View</a> 
  <a href="update-adoption-pet.php?adoption_number=<?php echo $row['adoption_number'] ?>" class="btn btn-success btn-md"><i class="fa-solid fa-pen-to-square"></i>Edit</a>
        <button type="button" onClick="delete_adoption_pet(<?php echo $row['adoption_number']; ?>)" class="btn btn-secondary btn-md"><i class="fa-solid fa-rotate-right"></i>Archive</button>
...
function delete_adoption_pet(delete_adoption_pet) {
    Swal.fire({
        title: 'Are you sure?',
        text: 'Unarchive adoption pet#' + delete_adoption_pet + '?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        ...
                        Swal.fire({
                          title: 'Unarchived!',
                          text: 'Adoption pet has been Unarchived.',
                          icon: 'success',
            ...
            xhr.send("adoption_number1=" + delete_adoption_pet);
        }
    })
}
</script>
