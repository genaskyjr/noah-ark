...
if(isset($_GET['search'])){
  $keyword = '%' . $_GET['search'] . '%';
  $stmt = $conn->prepare("SELECT `report_id`, `report_daytime`, `report_astray_image`, `report_astray_location`, `reporter_notes`, `reporter_name`, `reporter_email`, `reporter_number` ,claimed_by
  FROM `astray_reports` 
  WHERE 
      ...
      `claimed_by` LIKE :keyword OR
      `reporter_email` LIKE :keyword OR
      `reporter_number` LIKE :keyword) AND is_claim = 1;
  ");

$stmt->bindParam(':keyword', $keyword, PDO::PARAM_STR); // Bind the parameter

}else {
  $stmt = $conn->prepare("SELECT * FROM `astray_reports` WHERE is_claim = 1");
}
...
<tr>
    <td>
      ...
      <td><img  src="<?php echo $row['report_astray_image'];  ?>" alt="Logo" width="70" height="70" class="d-inline-block align-text-top"></td>
